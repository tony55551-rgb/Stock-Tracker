...
def get_relevant_news(ticker, name):
    """
    Refined News Logic: 
    Stage 1: Search for high-impact financial triggers.
    Stage 2: Fallback to general news if Stage 1 is empty.
    """
    ...
    query = f'("{core_name}" OR "{ticker.split(".")[0]}") AND (stock OR "order book" OR results OR breakout OR contract)'
    ...
    try:
        ...
        if not articles:
            # Stage 2: Fallback to general news
            url_fallback = f"https://newsapi.org/v2/everything?q={core_name}&sortBy=publishedAt&language=en&apiKey={api_key}"
            response = requests.get(url_fallback).json()
            articles = response.get('articles', [])
        ...
        return "Neutral catalyst: No major news found in last 48h."
    except:
        return "Intelligence feed currently offline."

def get_stock_intel(ticker, is_discovery=False):
    """Trend & Value Predictor: Identifies Golden Crosses."""
    try:
        ...
        is_value = isinstance(pe, (int, float)) and pe < 30 # Slightly higher for growth sectors
