...
def get_discovery_pool():
    ...
    Covers Semiconductors, Healthcare, Defense, Railways, and Green Energy.
    """
    return [
...
def get_relevant_news(ticker, name):
    """Refined News: Anchors search to financial catalysts and excludes noise."""
    api_key = os.getenv("NEWS_API_KEY")
    core_name = name.split(" ")[0]
    # Boolean logic to ensure financial relevance
    query = f'("{core_name}" OR "{ticker.split(".")[0]}") AND (stock OR "order book" OR results OR breakout OR "usfda") -jobs -hiring'
...
def get_stock_intel(ticker, is_discovery=False):
    """Trend & Value Predictor: Identifies Golden Crosses and Undervalued plays."""
    try:
        ...
        # 1. Trend: Golden Cross (50-day crossing above 200-day)
        ma50 = hist['Close'].rolling(50).mean().iloc[-1]
        ma200 = hist['Close'].rolling(200).mean().iloc[-1]
        is_breakout = ma50 > ma200 and price > ma50
        
        # 2. Fundamentals: PE < 25 (Standard threshold for growth)
        pe = inf.get('trailingPE')
        is_value = isinstance(pe, (int, float)) and pe < 25

        # AI Discovery Filter: Only include if it's a breakout or high value
...
def main():
    ...
    html += f"""<div style="background:{s_col}; color:white; padding:25px; border-radius:15px; text-align:center; margin-bottom:25px;">
        <h1 style="margin:0;">SENTIMENT: {sentiment}</h1>
        <p style="margin:8px 0 0;">Advance/Decline Ratio: {adr:.2f}</p>
    </div>"""
